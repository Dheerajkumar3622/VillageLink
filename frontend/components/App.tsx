...
const App: React.FC = () => {
  ...
  const renderRoleView = () => {
    ...
    switch (user.role) {
      ...
      case 'PASSENGER':
        return <UserApp user={user} onLogout={handleLogout} lang={lang} darkMode={darkMode} toggleTheme={toggleTheme} toggleLang={toggleLang} />;
      case 'DRIVER':
  ...
  if (appMode === 'USS_USER' || (user && user.role === 'PASSENGER')) {
    return (
      <div className="min-h-screen bg-obsidian-deep">
        <Suspense fallback={<ViewSkeleton />}>
          <UserApp user={user} onLogout={handleLogout} lang={lang} darkMode={darkMode} toggleTheme={toggleTheme} toggleLang={toggleLang} />
        </Suspense>
      </div>
    );
  }
