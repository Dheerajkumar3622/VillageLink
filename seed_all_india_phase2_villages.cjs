/**
 * VillageLink All-India Bulk Village Seeding Engine (Phase 2) - CJS
 * 
 * Expands database coverage from 4,75,014 nodes to 6,00,000+ villages across all 28 States & 8 UTs.
 * Adds 2DSphere spatial indexing and Uber H3 Resolution 7 & 9 cell tags.
 */
...
const MONGO_URI = process.env.MONGO_URI || 'mongodb://localhost:27017/villagelink';
...
// Regional State Generators for 100% All-India Coverage
const STATE_BOUNDS = [
  // South India
  ...
  for (const config of STATE_BOUNDS) {
    ...
    for (let i = 0; i < config.count; i++) {
      ...
      if (operations.length >= batchSize) {
        await VNISNodeModel.bulkWrite(operations, { ordered: false });
        totalNewInserted += operations.length;
        operations.length = 0;
      }
    }
