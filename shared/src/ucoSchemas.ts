import { UniversalCapacityObject, UniversalDemandObject, CoordinationUnit, CapacityEvent } from './types.js';

export function validateUCO(data: any): { valid: boolean; errors: string[]; uco?: UniversalCapacityObject } {
  const errors: string[] = [];
  ...
  return { valid: true, errors: [], uco: data as UniversalCapacityObject };
}

export function validateUDO(data: any): { valid: boolean; errors: string[]; udo?: UniversalDemandObject } {
  const errors: string[] = [];
  if (!data) return { valid: false, errors: ['Null or undefined payload'] };
  ...
  return { valid: true, errors: [], udo: data as UniversalDemandObject };
}
