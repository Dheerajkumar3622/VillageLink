import { UniversalDemandObject } from '../../shared/src/types.js';
import { SpatialTemporalIndexEngine } from './spatialTemporalIndex.js';
import { CapacityEventStore } from './eventStoreEngine.js';

export interface MerchantPickupPrediction {
  merchantId: string;
  expectedDispatchTime: number;
  predictedPackageCount: number;
...
export class MerchantOSEngine {
   ...
   */
  public static async publishNeedSignal(merchantData: {
    merchantId: string;
    merchantName: string;
    pickupLocation: { lat: number; lng: number; address: string };
    dropLocation: { lat: number; lng: number; address: string };
    itemType: 'Parcel' | 'AgriculturalGoods' | 'FoodDelivery' | 'Medicine';
    weightKg: number;
    volumeL: number;
    priority?: 'Low' | 'Medium' | 'High' | 'Critical';
    maxBudget?: number;
    pickupWithinMinutes?: number;
  }): Promise<UniversalDemandObject> {
    const demandId = `UDO_MERCHANT_${Date.now()}_${Math.floor(1000 + Math.random() * 9000)}`;
    const now = Date.now();
    const pickupWindowMs = (merchantData.pickupWithinMinutes || 60) * 60 * 1000;
    ...
    const udo: UniversalDemandObject = {
      demandId,
   ...
   */
  public static predictMerchantPickups(merchantId: string, historyCount: number = 10): MerchantPickupPrediction {
    const now = Date.now();
    ...
    return {
      ...
      predictedTotalWeightKg: Math.round(15.5 * (historyCount > 5 ? 1.5 : 1.0)),
